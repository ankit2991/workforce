...
// Handle launch queue - when PWA is launched via a URL (e.g. QR code scan),
// navigate the existing window to that URL instead of opening a new tab.
// This requires "launch_handler": {"client_mode": "navigate-existing"} in the manifest.
    ...
    if (launchParams.targetURL) {
      clients.matchAll({ type: "window" }).then((clientList) => {
        if (clientList.length > 0) {
          ...
          client.navigate(launchParams.targetURL);
          client.focus();
        } else if (clients.openWindow) {
          clients.openWindow(launchParams.targetURL);
        }
      });
    }
  });
